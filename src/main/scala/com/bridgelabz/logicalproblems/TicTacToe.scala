 ...
 * time - 23:37
 * package - com.bridgelabz.logicalproblems
 * title - Tic Tac Toe problem with all the use cases
 */
object TicTacToe {
  val Cell = 9
  var count = 0
  ...
  def tossForTurn(): Unit = {
    ...
    if (Random.nextInt(2) == 0) {
      ...
      var letter = Utilities.getStringInput("Choose your letter X or O: ")
      if (letter == "X" || letter == "O") {
        ...
        letter = Utilities.getStringInput("Please choose between X or O only: ")
      }
    }
    else {
  ...
  def playerIsPlaying(): Unit = {
    ...
    try {
      position = Utilities.getIntegerInput("Enter Position: ")
    } catch {
      case _: NumberFormatException => {
        position = Utilities.getIntegerInput("Enter Position in integer: ")
      }
    }
